...
def do_knn(k,c,new_t_set,new_t_tar):
    k_folds=10
    cv = cross_validation.KFold(len(new_t_tar), n_folds=k_folds, shuffle=True)  # Do 10-fold cross validation with shuffling
    classifier = neighbors.KNeighborsClassifier(n_neighbors=k)          # K nearest neighbor classifier
