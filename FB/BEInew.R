...
postData <- read.csv(paste0(name,"_1.txt"), header = FALSE)
...
for (i in 1:50) {
  ...
  ggplot(cmtBEI, aes(x=BEI)) + 
    ...
    ggsave(filename = paste0("/z/",name,"/",name,"_1_", "BEI_", i, ".pdf"), width = 8, height = 6)
  ...
  ggplot(cmtBEI, aes(x=Time)) + 
    ...
    ggsave(filename = paste0("/z/",name,"/",name,"_1_", "time_", i, ".pdf"), width = 8, height = 6)  
}

names(cmtStat) <- c("Sample", "BEI Mean", "BEI sd", "Time Mean", "Time sd")
cmtStat <- cbind(Comment = c(1:50) ,cmtStat)
write.csv(cmtStat, paste0("/z/",name,"/", name,"_1.csv"))
